    ...
    public JSONObject handle(ArrayList<String> id, Map<String, String> parameters, FullHttpRequest request, EgaSecureReEncryptionService ref) {
        ...
        try {
            ...
            if (function.equalsIgnoreCase("load")) {
                if (EgaSecureReEncryptionService.keepRunning)
                    result = new String[]{String.valueOf(EgaSecureReEncryptionService.getSystemCpuLoad())};
                else 
                    result = new String[]{"-2"}; // Indicate that service is shutting down
